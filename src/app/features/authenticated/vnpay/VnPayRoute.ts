...
const getOrderAmountInVnpUnit = async (
  ...
  if (!cartObjectId) return null;

  const cart = await cartRepo.findOne({
    where: { _id: cartObjectId },
  });

  if (!cart || typeof cart.finalPrice !== "number") return null;
...
router.post("/create_payment", (req: Request, res: Response) => {
  try {
    ...
    const secureHash = sign(sortedParams, secretKey);
...
router.get("/payment_ipn", async (req: Request, res: Response) => {
  try {
    ...
    const sortedParams = sortParams(params);
    const signed = sign(sortedParams, secretKey);
    ...
    const orderObjectId = toObjectId(params.vnp_TxnRef);
    if (!orderObjectId) {
